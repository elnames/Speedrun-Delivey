    ...
    <div className="min-h-screen bg-neutral-950 font-sans animate-fade-in pb-20">
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 space-y-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-8">
          <div>
            <h2 className="text-4xl font-bold tracking-tight text-white mb-2">Panel Ejecutivo</h2>
                    ...
                    <tr key={u.id} className="hover:bg-white/[0.03] transition-colors group">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-dark-700 to-dark-800 border border-white/10 flex items-center justify-center text-sm font-bold text-gray-400">
                            {u.nombre[0]}
                          </div>
                          <div>
                      ...
                      <td className="px-8 py-5 text-gray-400 font-mono text-xs italic">#{u.id}</td>
                      <td className="px-8 py-5">
                        <span className={`px-3 py-1 rounded-full text-[10px] font-bold border tracking-wider ${roleColors[u.role]}`}>{u.role}</span>
                      </td>
                      <td className="px-8 py-5 text-right space-x-2">
                        <button onClick={() => setEditingUser(u)} className="p-2 text-gray-400 hover:text-brand-light transition-colors"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                        <button onClick={() => setPassResetUser(u)} className="p-2 text-gray-400 hover:text-orange-400 transition-colors"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /></svg></button>
                        <button onClick={() => deleteUser(u.id)} className="p-2 text-gray-400 hover:text-red-400 transition-colors"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {tab === 'metrics' && metrics && (
          <div className="space-y-10 animate-slide-up">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="glass-premium p-8 group overflow-hidden relative border-brand/20">
                <div className="absolute top-0 right-0 w-24 h-24 bg-blue-500/10 rounded-full blur-3xl -mr-8 -mt-8" />
                <div className="text-4xl font-bold text-white mb-2 leading-none tracking-tighter">{metrics.total}</div>
                <div className="text-[10px] text-muted font-bold uppercase tracking-[0.2em] flex items-center gap-2">
                   <div className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" />
                   Total Operaciones
                </div>
              </div>
              <div className="glass-premium p-8 group overflow-hidden relative border-green-500/20">
                <div className="absolute top-0 right-0 w-24 h-24 bg-green-500/10 rounded-full blur-3xl -mr-8 -mt-8" />
                <div className="text-4xl font-bold text-white mb-2 leading-none tracking-tighter">{metrics.completed}</div>
                <div className="text-[10px] text-muted font-bold uppercase tracking-[0.2em] flex items-center gap-2">
                   <div className="w-1.5 h-1.5 rounded-full bg-green-400" />
                   Entregas Exitosas
                </div>
              </div>
              <div className="glass-premium p-8 group overflow-hidden relative border-purple-500/20">
                <div className="absolute top-0 right-0 w-24 h-24 bg-purple-500/10 rounded-full blur-3xl -mr-8 -mt-8" />
                <div className="text-4xl font-bold text-white mb-2 leading-none font-mono tracking-tight">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="glass-premium p-8">
                <h3 className="text-lg font-bold text-white mb-8 tracking-tight">Rendimiento Logístico</h3>
              ...
              <div className="glass-premium p-8">
                <h3 className="text-lg font-bold text-white mb-6 tracking-tight">Top Agentes</h3>
        ...
        {tab === 'reviews' && (
          <div className="animate-slide-up grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {reviews.map((r) => (
              <div key={r.id} className="glass-premium p-6 relative group border-transparent hover:border-white/10 transition-all">
                <button className="absolute top-6 right-6 text-gray-600 hover:text-red-400" onClick={() => deleteReview(r.id)}><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
      ...
      {isCreating && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="glass-premium w-full max-w-md p-8 animate-slide-up border-brand/40">
            <h3 className="text-xl font-bold text-white mb-6">Crear Nuevo Perfil</h3>
      ...
      {editingUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="glass-premium w-full max-w-md p-8 animate-slide-up">
            <h3 className="text-xl font-bold text-white mb-2">Editar Perfil</h3>
      ...
      {passResetUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="glass-card w-full max-w-md p-8 animate-slide-up border-orange-500/40">
            <h3 className="text-xl font-bold text-white mb-2">Reasignar Contraseña</h3>
